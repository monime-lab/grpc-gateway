...
1. Write a custom implementation
   of [`Marshaler`](https://pkg.go.dev/github.com/monime-lab/grpc-gateway/runtime?tab=doc#Marshaler).

2. Register your marshaler
   with [`WithMarshalerOption`](https://pkg.go.dev/github.com/monime-lab/grpc-gateway/runtime?tab=doc#WithMarshalerOption)
   .
...
You can
see [the default implementation for JSON](https://github.com/monime-lab/grpc-gateway/blob/master/runtime/marshal_jsonpb.go)
for reference.
...
You might not
like [the default mapping rule](https://pkg.go.dev/github.com/monime-lab/grpc-gateway/runtime?tab=doc#DefaultHeaderMatcher)
and might want to pass through all the HTTP headers, for example:

1. Write
   a [`HeaderMatcherFunc`](https://pkg.go.dev/github.com/monime-lab/grpc-gateway/runtime?tab=doc#HeaderMatcherFunc).

2. Register the function
   with [`WithIncomingHeaderMatcher`](https://pkg.go.dev/github.com/monime-lab/grpc-gateway/runtime?tab=doc#WithIncomingHeaderMatcher)
...
To keep
the [the default mapping rule](https://pkg.go.dev/github.com/monime-lab/grpc-gateway/runtime?tab=doc#DefaultHeaderMatcher)
...
Use [`WithOutgoingHeaderMatcher`](https://pkg.go.dev/github.com/monime-lab/grpc-gateway/runtime?tab=doc#WithOutgoingHeaderMatcher)
...
2. Register the filter
   with [`WithForwardResponseOption`](https://pkg.go.dev/github.com/monime-lab/grpc-gateway/runtime?tab=doc#WithForwardResponseOption)
...
By default, gRPC-Gateway unescapes the entire URL path string attempting to route a request. This causes routing errors
when the path parameter contains an illegal character such as `/`.
...
use [runtime.WithUnescapingMode()](https://pkg.go.dev/github.com/monime-lab/grpc-gateway/runtime?tab=doc#WithUnescapingMode)
